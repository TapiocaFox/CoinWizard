...
def start():
    ...
    while True:
        ...
        if answer == 99:
            ...
            states["latest_broker_plot_settings"]["granularity"] = granularity = session.prompt("  Granularity (M1/M5/M15/M30/H1/H4/D): ", default=str(states["latest_broker_plot_settings"]["granularity"]))
            if pair.islower():
            ...
            granularity = states["latest_broker_plot_settings"]["granularity"]
            if pair.islower():
                pair = translate_pair_to_splited(pair)
            instrument = broker_platform.getInstrument(pair)
            print('')
