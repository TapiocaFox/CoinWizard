...
def time_features(dates_series, device, freq='T'):
    ...
    return torch.tensor([feat(dates).to_list() for feat in time_features_from_frequency_str(freq)], device=device).transpose(1, 0)
...
class AppendTimeFeaturesPipline(Pipeline):
    ...
    def setupInSpecs(self, in_specs):
        ...
        for feature in self.freq_features:
            self.out_specs['tensor_feature_description_list'].append({'type': 'time_feature', 'feature': feature.__name__})
