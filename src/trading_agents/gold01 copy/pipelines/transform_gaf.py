...
class TransformGafPipeline(Pipeline):
    def __init__(self, method='summation', features_to_channels=True):
        super().__init__(torch.Tensor, torch.Tensor)
        self.method = 1 if method in ['difference', 'd'] else 0
    ...
    def _process(self, in_data, attachment_dict):
        ...
        # GASF
        if self.method == 0:
            x = self._gasf(x_cos, x_sin)
        # GADF
        else:
            x = self._gadf(x_cos, x_sin)
