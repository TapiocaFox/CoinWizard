...
class HistDataGym(Gym):
    ...
    def start(self):
        ...
        for i in range(episode_steps):
            ...
            # After action taken. Still "plus one" since we assume that the action is immediately applied after observation.
            known_hist_data = hist_data.iloc[:(latest_known_index+1)]
            reward, trade = self.reward_calculator.calculateReward(known_hist_data, action)
