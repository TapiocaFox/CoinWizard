...
class DataLoader(object):
    def __init__(self, instrument_list, primary_intrument, granularity_list=None, primary_granularity=None, input_period_list=None, from_datetime=None, to_datetime=None, episode_steps=None, cuda=False):
        ...
        for granularity_index, granularity in enumerate(granularity_list):
            primary_hist_data = hist.get_historical_pair_data_pandas(instrument_list[primary_intrument], from_datetime, to_datetime, granularity=granularity, invert=False)
            primary_hist_data = primary_hist_data.dropna().reset_index()
            for instrument_index, instrument in enumerate(instrument_list):
                ...
                # Merge
                primary_hist_data = primary_hist_data.merge(hist_data, how='left', on='timestamp', suffixes=(None, str(instrument_index)))

            primary_hist_data = primary_hist_data.fillna(0)

            self.hist_data_granularities[granularity_index] = primary_hist_data

            tensor = from_dataframe_to_tensor(instrument_list, primary_intrument, primary_hist_data, self.device)
            # print(tensor.type())
            ...
            timestamp_series_granularities[granularity_index] = primary_hist_data.timestamp
