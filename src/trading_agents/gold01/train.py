...
from pipelines.append_culr import AppendCulrPipeline
from pipelines.append_position import AppendPositionPipeline
from pipelines.remove_type import RemoveTypePipeline
from pipelines.transform_min_max import TransformMinMaxPipeline
...
from reward_calculators.every_step import EveryStepRewardCalculator
...
hist_data_gym = HistDataGym(
                    ...
                    train_to_datetime,
                    episode_steps,
                    EveryStepRewardCalculator(primary_instrument_spread),
                    AccountBook()
                )
...
pa = PipelineArray([
    ...
    TransformMinMaxPipeline(3, None, None),
    PrinterPipeline(torch.Tensor),
    # RemoveTypePipeline('ohlc'),
    # PrinterPipeline(torch.Tensor),
    TransformGafPipeline(features_to_channels=True),
    # PrinterPipeline(torch.Tensor),
    ], gym_specs)
...
def on_step_callback(finished, state):
    ...
    action = 1
    pa.pipeline_list[1].pushPosition(action)
