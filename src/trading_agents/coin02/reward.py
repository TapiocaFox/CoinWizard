...
class RewardCalculator(object):
    def __init__(self, trading_amount_rate=0.5, spread=0.0002, cuda=False):
    ...
    def step(self, current_episode_steps, actions, rewards_for_all_actions=True):
        ...
        if rewards_for_all_actions:
            ...
            # print(spread_terms)
            # print(trading_amount_rate*self.all_actions_tensor*price_diff.unsqueeze(1))
            # print(v)
            # print((v[:, 0]+v[:, 2])/2)
            # raise
            # print(spread_terms.shape)
            # print((trading_amount_rate*self.all_actions_tensor*price_diff.unsqueeze(1)).shape)
            # v = - spread_terms
            v = trading_amount_rate*self.all_actions_tensor*price_diff.unsqueeze(1) - spread_terms
            # print(v)
            # print(torch.log(v))
            return v
            # return torch.log(v)
